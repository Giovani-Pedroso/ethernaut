...
import { ABI, ABI_ATTACK } from "./variables/07/ABIS";
...
if (TEST) {
  ...
  CONTRACT_ADDS = process.env.FORCE_ADDRS as string;
...
const main = async () => {
  ...
  // contract_att.deposity({ value: valueToSend });
  // contract_att.end(CONTRACT_ADDS);
  // const balance = await provider.getBalance(CONTRACT_ADDS_ATTACK);
